...
public class ChessBoard : MonoBehaviour
{
    ...
    [SerializeField] private float deathSize = 0.0375f;
        ...
        if (Physics.Raycast(ray, out info, 100, LayerMask.GetMask("Tile", "Hover", "Highlight")))
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                if (chessPieces[hitPosition.x, hitPosition.y] != null)
                {
                    ...
                    if (chessPieces[hitPosition.x, hitPosition.y].team == 1 && !isWhiteTurn)
                    {
                        currentlyDragging = chessPieces[hitPosition.x, hitPosition.y];
                        ...
                        /*float randMoveX = UnityEngine.Random.Range(TILE_COUNT_X, TILE_COUNT_Y);
                        float randMoveY = UnityEngine.Random.Range(TILE_COUNT_X, TILE_COUNT_Y);
                        Vector2 target = new Vector2(randMoveX, randMoveY);*/

                        //chessPieces[0, 0].transform.position = new Vector3(0, 1, 0);
    ...
    private GameObject GenerateSingleTile(float tileSize, int x, int y)
    {
        ...
        tileObject.AddComponent<MeshRenderer>().material = squareMaterial;
    ...
    {
        NetUtility.S_WELCOME += OnWelcomeSever;
    ...
    private void OnWelcomeClient(NetMessage message)
    {
        ...
        playerteam = netWelcome.SetSide;
        Debug.Log($"The team you are on is {netWelcome.SetSide}");


    }

    private void OnWelcomeSever(NetMessage message, NetworkConnection connection)
    {
        NetWelcome netWelcome = message as NetWelcome;
        netWelcome.SetSide = ++playernumber;
        Server.instance.SendToClient(connection, netWelcome);
        if(playernumber == 1)
        {
            Server.instance.Broadcast(new Netstartthegame());
        }
    }

    private void UnLogActions()
    {
